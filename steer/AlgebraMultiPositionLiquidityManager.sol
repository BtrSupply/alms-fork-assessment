...
import "./AlgebraBaseLiquidityManager.sol";

contract AlgebraMultiPositionLiquidityManager is
  AlgebraBaseLiquidityManager
{
  ...
  function _setBins(
    ...
    for (uint256 i; i != positionCount; ++i) {
      ...
      if (i >= 1) {
        require(
          _positions.lowerTick[i - 1] < _positions.lowerTick[i] &&
    ...
    for (uint256 i; i != positionCount; ++i) {
      ...
      // Slightly tortured logic here due to stack management
      {
        ...
        liquidityToBurn = uint128(
          FullMath.mulDiv(
            uint256(totalPositionLiquidity),
            shares,
            totalShares
          )
        );

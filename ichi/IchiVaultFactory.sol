...
contract ICHIVaultFactory is IICHIVaultFactory, ReentrancyGuard, Ownable {
  ...
  constructor(address _uniswapV3Factory) {
    require(
      _uniswapV3Factory != NULL_ADDRESS,
      "IVF.constructor: zero address"
    );
  ...
  function createICHIVault(
    ...
    require(
      allowTokenA || allowTokenB,
      "IVF.createICHIVault: no allowed tokens"
    );
    ...
    int24 tickSpacing = IPancakeV3Factory(uniswapV3Factory)
      .feeAmountTickSpacing(fee);
    require(tickSpacing != 0, "IVF.createICHIVault: fee incorrect");
    address pool = IPancakeV3Factory(uniswapV3Factory).getPool(
      tokenA,
      tokenB,
      fee
    );
    ...
    require(
      observationCardinalityNext >= MIN_OBSERVATIONS,
      "IVF.createICHIVault: observation cardinality too low"
    );
    ...
    ] = ichiVault; // populate mapping in the reverse direction
    allVaults.push(ichiVault);

    emit ICHIVaultCreated(
      msg.sender,
      ichiVault,
      token0,
      allowToken0,
  ...
  function setFeeRecipient(
    ...
    require(
      _feeRecipient != NULL_ADDRESS,
      "IVF.setFeeRecipient: zero address"
    );
  ...
  function setBaseFeeSplit(
    ...
    require(
      _baseFeeSplit <= PRECISION,
      "IVF.setBaseFeeSplit: must be <= 10**18"
    );

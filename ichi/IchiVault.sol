  ...
  Ownable
{
  ...
  function deposit(
    ...
    if (baseLiquidity > 0) {
      (uint burn0, uint burn1) = IPancakeV3Pool(pool).burn(baseLower, baseUpper, 0);
      require(burn0 == 0 && burn1 == 0, "IV.deposit: unexpected burn (1)");
    }

    (uint128 limitLiquidity, , ) = _position(limitLower, limitUpper);
    if (limitLiquidity > 0) {
      (uint burn0, uint burn1) = IPancakeV3Pool(pool).burn(limitLower, limitUpper, 0);
  ...
  function rebalance(
    ...
    (uint128 baseLiquidity, , ) = _position(baseLower, baseUpper);
    if (baseLiquidity > 0) {
      IPancakeV3Pool(pool).burn(baseLower, baseUpper, 0);
    }
    (uint128 limitLiquidity, , ) = _position(limitLower, limitUpper);
    if (limitLiquidity > 0) {
      IPancakeV3Pool(pool).burn(limitLower, limitUpper, 0);
    }
  ...
  function _mintLiquidity(
    ...
    if (liquidity > 0) {
      (amount0, amount1) = IPancakeV3Pool(pool).mint(
        address(this),
        tickLower,
        tickUpper,
        liquidity,
  ...
  function _burnAnyLiquidity(
    ...
    // Burn liquidity
    (uint256 owed0, uint256 owed1) = IPancakeV3Pool(pool).burn(
      tickLower,
      tickUpper,
      liquidity
    );
    ...
    if (collect0 > 0 || collect1 > 0) {
      (amount0, amount1) = IPancakeV3Pool(pool).collect(
        to,
        tickLower,
        tickUpper,
        collect0,
    ...
    )
  {
    ...
    (liquidity, , , tokensOwed0, tokensOwed1) = IPancakeV3Pool(pool)
  ...
  function checkHysteresis() private view returns(bool) {
    (, , uint16 observationIndex, , , , ) = IPancakeV3Pool(pool).slot0();
    (uint32 blockTimestamp, , ,) = IPancakeV3Pool(pool).observations(observationIndex);
  ...
  function _amountsForLiquidity(
    ...
    (uint160 sqrtRatioX96, , , , , , ) = IPancakeV3Pool(pool).slot0();
    return
  ...
  function currentTick() public view returns (int24 tick) {
    (, int24 tick_, , , , , bool unlocked_) = IPancakeV3Pool(pool).slot0();
